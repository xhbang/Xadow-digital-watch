...
static display_t draw_normal(void);
...
{
	display_setDrawFunc(draw_normal);
...
{   
	...
	// 
	//shao if(stopwatch_active())
	//shao {
	//shao 	s_image stopwatchIcon = {x, FRAME_HEIGHT - 8, stopwatch, 8, 8, WHITE, NOINVERT, 0};
	//shao 	draw_bitmap_s2(&stopwatchIcon);
	//shao 	//draw_string("X", NOINVERT, x, FRAME_HEIGHT - 7);
	//shao 	x += 12;
	//shao }

	// Draw next alarm
	s_alarm* nextAlarm = alarm_getNext();return busy;
	if(nextAlarm != NULL)
	{
		char buff[8];
